...
\end{align*}
\indices{m0:pop:left}{ r : \REQ }
% theorem prover found that qe.(q-1) (from the event pop:right), should be 
% changed for qe.p
\begin{align*}
\cschedule{m0:pop:left}{m1:sch0}{ r \in popL }
% \\ \removecoarse{m0:pop:left}{default} % \weakento{m0:pop:left}{default}{m1:sch0}
...
\begin{align*}
\invariant{m2:inv2}{ \qforall{i}{ \betweenR{p}{i}{q} }{ ~ qe.i = val.(obj.i) } }
\end{align*}

Notice that $i$ has not been declared as a dummy at this point; its type is simply inferred from the context.

Note: Invariant \ref{m2:inv3} has been dictated (at this point of writing the model) by a WD proof obligation.
...
\ref{m2:inv0}:
\begin{align*}
\evbcmeq{m0:push:left}{m2:a0}{obj}{ obj | (p-1) \fun n }
\\ \evbcmeq{m0:push:left}{m2:a1}{node}{ node \bunion \{ n \} }
\end{align*}

\ref{m2:inv1}: skips on $val$ and $live$
% \begin{align*}
...
\end{align*}
\replace{m0:push:left}{}{m2:sch0,m2:sch1}{m1:sch0}{m2:prog0}{m2:saf0}
\begin{align*}
\progress{m2:prog0}
	{ r \in \dom.pshL }
...
\begin{proof}{\ref{m0:push:left}/SAF/\ref{m2:saf0}}
	...
	\begin{case}{hyp1}{\neg r = r_0}
		...
		% 	pshL'.r_0 \in \ran.((node\domsub val) \finter (\{ n \}\domsub val))
		\end{calculation}
		\end{part:a}
		\end{by:parts}
	\end{case}		
...
% \\ \evbcmeq{m0:pop:right}{m2:a3}{ live' = live }
% \end{align*}
...
\begin{case}{pr2:hyp1}{\neg p = q}
	...
	\hint{=}{  }
		insL.r_0 \in (\dom.val \setminus node) \bunion (\dom.val \binter \{ obj.(q\0-1) \})
...
\begin{align*}
% \progress{m2:prog0}{ r \in \dom.pshL }{ r \in \dom.pshL \land pshL.r \in \ran.val }
