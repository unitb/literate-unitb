{-# LANGUAGE LambdaCase,TypeOperators
        ...
        ,ScopedTypeVariables
        ,StandaloneDeriving #-}
module Logic.Expr.Bound
    ( module Logic.Expr.Bound
    , module Bound)
where

import Bound hiding (Var,substitute)
import Bound.Var 
...
newtype Scope' f b a = Scope' (Scope b f a)
...
instance Bifunctor (Scope' f) where
    bimap = bimapDefault
instance Bifoldable (Scope' f) where
    bifoldMap = bifoldMapDefault
instance Bitraversable (Scope' f) where
...
instance (ZoomEq a,ZoomEq b) => ZoomEq (Var a b) where

instance (Eq1 expr,Show1 expr,Monad expr,ZoomEq lv,ZoomEq v,Lifting ZoomEq expr) 
        => ZoomEq (Scope lv expr v) where
    x .== y = unlift l $ fromScope x .== fromScope y
        where
            l = lifting :: ZoomEq (Var lv v) :- ZoomEq (expr (Var lv v))

liftedArbitrary :: Arbitrary a
           => Arbitrary a :- Arbitrary (expr a)
           -> Gen (expr a)
liftedArbitrary (Sub Dict) = arbitrary
...
instance (Monad expr,Lifting Arbitrary expr,Arbitrary lv,Arbitrary v) => Arbitrary (Scope lv expr v) where
    arbitrary = toScope <$> liftedArbitrary lifting
