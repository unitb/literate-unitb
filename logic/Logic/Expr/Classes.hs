...
insert_symbol :: IsKey Map n => HasName a n => a -> Map n a -> Map n a
insert_symbol x = M.insert (x^.name) x

symbol_table' :: (HasName b n, Foldable f,IsKey Map n) 
              => (a -> b) -> f a -> Map n a
symbol_table' f xs = M.fromList $ L.map (as_pair' f) $ F.toList xs

symbol_table :: (HasName a n, Foldable f,M.IsKey Map n) 
             => f a -> Map n a
