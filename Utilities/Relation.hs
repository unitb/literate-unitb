...
images :: Ord a => Relation a b -> a -> S.Set b
...
identity :: S.Set a -> Relation a a
identity s = Rel $ M.fromSet (`M.singleton` ()) s

(<|)  :: Ord a => S.Set a -> Relation a b -> Relation a b
(<<|) :: Ord a => S.Set a -> Relation a b -> Relation a b
(|>)  :: Ord b => Relation a b -> S.Set b -> Relation a b
(|>>) :: Ord b => Relation a b -> S.Set b -> Relation a b
...
bimapMaybe :: (Ord a1,Ord b0,Ord b1)
           => (a0 -> Maybe a1)
           -> (b0 -> Maybe b1)
           -> Relation a0 b0
           -> Relation a1 b1
...
mapMaybeDom :: (Ord b,Ord c) => (a -> Maybe c) -> Relation a b -> Relation c b
mapMaybeDom p (Rel m) = Rel $ M.fromListWith M.union $ mapMaybe p' $ M.toList m
    where
        p' (x,y) = p x >>= \x -> return (x,y)

mapMaybeRan :: Ord c => (b -> Maybe c) -> Relation a b -> Relation a c
...
prop_range_def :: Ord b => Relation a b -> Bool
...
prop_identity_def :: Eq b => S.Set b -> Bool
prop_identity_def s = toList (identity s) == map (\x -> (x,x)) (S.toList s)

prop_ranSubt :: (Ord b, Eq a) => Relation a b -> S.Set b -> Bool
...
prop_transpose_def :: (Ord b, Ord a) => Relation b a -> Bool
...
prop_mapMaybeDom_def :: (Ord c, Ord b) => Fun b1 (Maybe c) -> Relation b1 b -> Bool
prop_mapMaybeDom_def (Fun _ f) r = toList (mapMaybeDom f r) == LO.nubSort (mapMaybe (first' f) (toList r))
    where
        first' = runKleisli . first . Kleisli

prop_mapMaybeRan_def :: (Ord c, Ord a) => Fun b (Maybe c) -> Relation a b -> Bool
...
prop_bimapMaybe_def :: (Ord a1, Ord b0, Ord b1) => Fun a0 (Maybe a1) -> Fun b0 (Maybe b1) -> Relation a0 b0 -> Bool
